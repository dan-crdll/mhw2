...
for (let a of document.querySelectorAll(".choice-grid div")) {
...
function selectAnswer(event) {
  ...
  if (selectedAnswers.includes(null))
    //se ancora non sono state selezionate risposte per ogni domanda
    checkForSelection(selected);
}

function checkForSelection(selected) {
  switch (
    selected.dataset.questionId //switch sul numero di domanda
  ) {
    case "one":
      select(selected, 0);
      break;
...
function select(selected, question) {
  if (selectedAnswers[question] !== null) {
    //sostituisco la risposta selezionata per la domanda
    selectedAnswers[question].querySelector(".checkbox").src =
  ...
  const selectString =
    '[data-question-id="' + selected.dataset.questionId + '"]';

  for (let a of document.querySelectorAll(selectString)) {
    //seleziono tutte le risposte di quella domanda
    if (a !== selected) {
  ...
  if (!selectedAnswers.includes(null))
    //se sono state selezionate risposte per ogni domanda
    showResults();
}
...
function findResult(title, content) {
  ...
  for (let a of selectedAnswers) results.push(a.dataset.choiceId);
  ...
  if (result === undefined) result = results[0];
